window.onload = function() {
    // This entire block of code will only execute after all page resources,
    // including all external scripts, have fully loaded.
    // This is our definitive fix for the "THREE is not defined" error.
    ...
    try {
        ...
        // --- Main execution ---
        function init() {
            // We double-check that THREE exists before using it.
            if (typeof THREE === 'undefined') {
                console.error("FATAL: Three.js library did not load correctly.");
                return; // Stop if the library is missing
            }
            init3D();
            initBackground();
            animate();
        ...
        console.error("A critical error occurred. Please report this:", e);
        ...
        document.body.innerHTML = `<div style="color: white; font-family: monospace; padding: 20px;">
            <h1>An error occurred</h1><p>Could not initialize the 3D scene. Please check the console for details.</p>
            <p>Error: ${e.message}</p>
        </div>`;
    }
};
